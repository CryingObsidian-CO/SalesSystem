    ...
    ui(new Ui::HistoryDialog)
{
    ...
    // 手动连接信号和槽
    connect(ui->transactionTable, &QTableView::doubleClicked, this, &HistoryDialog::on_transactionTable_doubleClicked);
    connect(ui->refreshButton, &QPushButton::clicked, this, &HistoryDialog::on_refreshButton_clicked);
...
void HistoryDialog::showTransactionDetails(const int transactionId)
{
...
{
    auto lowStockProducts = get_low_stock_products(10);
...
void HistoryDialog::showLowStockWarning()
{
    ...
    {
        warningText += QString::fromStdString(product.name) + ": 库存 " + QString::number(product.stock) + "\n";
    }
